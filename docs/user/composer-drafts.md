...
the same styal environment. The selected model, runtime mode, and interaction mode travel with
...
While an existing-thread draft contains any of that context, styal withholds the whole draft from
...
Drafts are stored by the styal server for that environment. They are not shared between separate
