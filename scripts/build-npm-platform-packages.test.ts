...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub `styal` that echoes its args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "styal-npm-packages-test-" });
  const archivesDir = path.join(root, "archives");
  yield* fs.makeDirectory(archivesDir);
  for (const key of KEYS) {
    const stem = `styal-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "styal"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "styal"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@styal/cli-darwin-arm64", "@styal/cli-linux-x64", "@styal/cli"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@styal/cli-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@styal/cli-linux-x64");
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@styal"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@styal/cli-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/styal$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "@styal/cli.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/styal.js", "--version"], {
        cwd: launcherDir,
      ...
      assert.include(unsupported.stderr, "https://github.com/incognitojam/styal/releases");
    }),
  );
});
