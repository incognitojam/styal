...
This guide is for people who want to use more than one Codex account in styal. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install styal](./install.md).
...
issue. styal uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
...
reports them. If Codex does not report either value, styal leaves it out instead of using the
parent agent's settings.

## Approve access to other apps

When a Codex tool needs access to an app such as Safari, styal shows the app name and asks for
...
In styal Settings, name it something obvious:
...
In styal Settings, add another Codex provider:
...
styal shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, styal treats it
...
reports a disruption, styal shows affected services above **Settings** in the web and desktop
