...
This guide is for people who want to use more than one Claude setup in styal. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install styal](./install.md).
...
In styal Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means styal uses Claude Code's normal config directory.

When you set this field, styal points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, styal
...
styal looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where styal looks.

Then add another Claude provider in styal:
...
styal only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so styal keeps separate config directories isolated
...
Add or edit a Claude provider in styal Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. styal stores the value as a server secret and does not
...
styal does not need a special Claude Code Router provider. Treat the router as a Claude
...
Anthropic reports a Claude service disruption, styal shows the affected services above
