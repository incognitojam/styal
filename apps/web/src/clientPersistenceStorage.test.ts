...
describe("clientPersistenceStorage", () => {
  ...
  it.each(["not-json", '{"wordWrap":"invalid"}'])(
    ...
    async (value) => {
      ...
      testWindow.localStorage.setItem("styal:client-settings:v1", value);
      ...
      expect(testWindow.localStorage.getItem("styal:client-settings:v1")).toBe(value);
    },
  );
  ...
  it("preserves saved settings across a transient read failure", async () => {
    ...
    testWindow.localStorage.setItem("styal:client-settings:v1", JSON.stringify(settings));
  ...
  it("keeps the diff layout across reloads and defaults it to stacked", async () => {
    ...
    testWindow.localStorage.setItem("styal:client-settings:v1", JSON.stringify({}));
