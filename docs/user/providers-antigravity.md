...
styal runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for styal to confirm
...
hostname. Only that styal sign-in session can finish the attempt. If it expires,
...
styal keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. styal also reads `.gemini/skills` and the
