#!/bin/sh
# Installs the styal CLI from a GitHub Release archive. Needs only sh, tar,
# sha256sum or shasum, and curl or wget; no Node, npm, or compiler.
#
#   curl -fsSL https://raw.githubusercontent.com/incognitojam/styal/main/scripts/install.sh | sh
#
# Environment:
...
#   STYAL_INSTALL_BIN_DIR   where the `styal` symlink goes (default: ~/.local/bin)
...
base_url="${STYAL_RELEASE_BASE_URL:-https://github.com/${repo}/releases/download}"
styal_home="${STYAL_HOME:-$HOME/.styal}"
bin_dir="${STYAL_INSTALL_BIN_DIR:-$HOME/.local/bin}"
...
channel="${STYAL_CHANNEL:-stable}"
version="${STYAL_VERSION:-}"
if [ -z "$version" ]; then
  ...
  case "$channel" in
    ...
    *) fail "STYAL_CHANNEL must be stable, nightly, or preview" ;;
  esac
  tmp_index="$(mktemp)"
  ...
  rm -f "$tmp_index"
  [ -n "$version" ] || fail "could not find a ${channel} release; set STYAL_VERSION"
fi
case "$version" in
  *-preview.*)
    printf '%s\n' \
      "styal ${version} is a preview build." \
      ...
      "  Set STYAL_CHANNEL=stable (the default) for a supported build." >&2
    if [ "$channel" != "preview" ] && [ -z "${STYAL_VERSION:-}" ]; then
      fail "refusing a preview build that was not explicitly requested"
    fi
    ;;
esac

stem="styal-${version}-${platform}-${arch}"
archive="${stem}.tar.gz"
versions_dir="${styal_home}/runtime/styal-executable/versions"
...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  printf '  %sInstalling%s styal %s%s%s\n\n' "$muted" "$reset" "$bold" "$version" "$reset" >&2
  step "Downloading..."
  fetch_status=0
  fetch "${base_url}/v${version}/SHA256SUMS" "${staging}/SHA256SUMS" || fetch_status=$?
  if [ "$fetch_status" -eq 44 ]; then
    fail "styal ${version} has no release archive for ${platform}-${arch}; releases before the self-contained CLI can only be installed with \`npm install -g @styal/cli@${version}\`"
  ...
  "${staging}/styal" --version >/dev/null || fail "the downloaded executable does not run"
...
mkdir -p "$bin_dir"
ln -sfn "${target_dir}/styal" "${bin_dir}/styal"
if "$interactive"; then printf '\r\033[2K' >&2; fi
printf '  %sInstalled styal %s%s\n\n' "$green" "$version" "$reset" >&2
case ":${PATH}:" in
  *":${bin_dir}:"*) printf '  Run %sstyal%s to get started.\n\n' "$bold" "$reset" ;;
  *) printf '  Add %s to your PATH, then run %sstyal%s.\n\n' "$bin_dir" "$bold" "$reset" ;;
esac
