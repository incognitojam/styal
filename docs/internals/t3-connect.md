...
Local desktop development uses `styal-dev://app`, while packaged builds use `styal://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `styal-dev://app`; the production Clerk instance should only need
`styal://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `build.styal.app`. To enable native passkeys:

1. Create an explicit macOS App ID for `build.styal.app` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.build.styal.app`.
