...
export const KDE_CAPTURE_EXECUTABLE = "styal-kde-snap-shot";
const DESKTOP_FILE = "build.styal.Styal.KdeCapture.desktop";
const MARKER = "X-Styal-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "styal", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall styal.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall styal.",
        );
