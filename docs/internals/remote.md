...
| `RelayConnectionTarget`   | Managed styal Link relay tunnels.                                        |
...
Managed styal Link relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
- **Client-managed local publish.** A local server is published through the relay with
  `styal link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
