...
- **Upstream is read-only and never auto-synced.** `main` is not rebased onto `pingdotgg/t3code`. Upstream changes are brought in deliberately, one or more at a time. Routine candidates use `intake/**` branches based on `main`; follow `docs/operations/upstream-tracking.md` for the current validation and promotion boundary. Never rebase `main` onto upstream, never force-push it, and never open a PR that assumes upstream's tip rather than `main`.
- Preserve upstream intake provenance in landed commit metadata. Intake commits use comma-separated `Upstream-PR: 1234, 5678` and/or `Upstream-Commit: <full lowercase SHA>` trailers; fork pull requests retain their source references and commit trailers in the squash commit body. The promotion workflow's `source_prs` and `source_commits` inputs must exactly match that metadata so the tracking issue can reconcile landed work without notifying upstream. Review empty PR merge diffs against the actual source changes; never assume a preceding commit belongs to that PR.
