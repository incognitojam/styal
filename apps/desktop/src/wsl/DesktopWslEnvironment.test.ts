...
// Stands in for the release's self-contained `styal` executable: the install
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/styal/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.styal/wsl-runtime"');
    ...
    // The release archive wraps everything in one `styal-<version>-linux-x64/`
    // directory; stripping it puts the executable at `$runtime_root/styal`.
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/styal/wsl-runtime.tar.gz' -C \"$runtime_tmp\" --strip-components=1",
    );
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/styal/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  it("treats a runtime whose executable no longer runs as a cache miss", () => {
    ...
    expect(script).toContain('  [ -x "$1/styal" ] && "$1/styal" --version >/dev/null 2>&1');
  ...
  // A swapped or half-written `styal` can still exist and even still answer
  ...
  it("re-hashes the executable against the digest the install recorded", () => {
    ...
    expect(script).toContain(`  sha256sum "$1/styal" 2>/dev/null | cut -d ' ' -f 1`);
    expect(script).toContain(
    ...
    expect(script).toContain(
      `printf '%s\\n' "$installed_entry_digest" > "$runtime_tmp/.styal-wsl-runtime-ready"`,
    );
    ...
    const markerWritten = script.indexOf('> "$runtime_tmp/.styal-wsl-runtime-ready"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.styal/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.styal/wsl-runtime/1.2.3-x64",
    );
  ...
  it("prunes completed runtimes except the current and newest previous cache", () => {
    ...
    expect(script).toContain('[ -f "$candidate/.styal-wsl-runtime-ready" ] || continue');
  ...
  it("never deletes a runtime another backend is running from", () => {
    ...
    // The running backend's argv holds `<runtime>/styal`, so
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain('rm -f "$HOME/.styal/wsl-runtime/1.2.3_x64/.styal-wsl-runtime-ready"');
  ...
  const createFixture = () => {
    const result = runShell(
      [
        ...
        'stage="$work/stage/styal-0.0.0-linux-x64"',
        'mkdir -p "$stage/node_modules/node-pty/build/Release" "$work/home"',
        `printf '%s' ${sh(SERVER_ENTRY_SOURCE)} > "$stage/styal"`,
        'chmod +x "$stage/styal"',
        `printf '%s' 'pty-native-payload' > "$stage/node_modules/node-pty/build/Release/pty.node"`,
        `tar -czf "$work/wsl-runtime.tar.gz" -C "$work/stage" styal-0.0.0-linux-x64`,
        `printf 'work:%s\\n' "$work"`,
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.styal/wsl-runtime`,
      runtimeRoot: `${work}/home/.styal/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.styal/wsl-runtime/${runtimeId}/styal`,
      installScript,
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-current/.styal-wsl-runtime-ready"',
        'printf ready > "$runtime_parent/sha256-previous/.styal-wsl-runtime-ready"',
  ...
  it("prunes a selected cache after its prepare-to-spawn grace period expires", () => {
    ...
    const result = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-previous/.styal-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        `touch -d "10 minutes ago" ${sh(`${fixture.runtimeRoot}/.styal-wsl-runtime-selected`)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `HOME=${sh(`${fixture.work}/home`)}`,
        "export HOME",
        buildWslRuntimePruneScript("sha256-current"),
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        `runtime_parent=${sh(fixture.runtimeParent)}`,
        'rm "$runtime_root/.styal-wsl-runtime-ready"',
        'sh -c "sleep 30" "$runtime_root/styal" >/dev/null 2>&1 &',
        "active_pid=$!",
        "sleep 0.1",
        fixture.installScript(),
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.styal/wsl-runtime"',
        'mkdir -p "$runtime_parent"',
        'make_ready() { mkdir -p "$runtime_parent/$1"; printf ready > "$runtime_parent/$1/.styal-wsl-runtime-ready"; }',
        ...
        'sh -c "sleep 30" "$runtime_parent/sha256-active/styal" >/dev/null 2>&1 &',
        "active_pid=$!",
        "(",
