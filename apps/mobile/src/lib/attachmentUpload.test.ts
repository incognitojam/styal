...
describe("prepareTurnAttachments", () => {
  it("rejects oversized mixed batches before any upload is minted", async () => {
    await expect(
      prepareTurnAttachments({
        environmentId,
        attachments: [
          { ...image, sizeBytes: 10 * 1024 * 1024 },
          { ...file, sizeBytes: 40 * 1024 * 1024 + 1 },
        ],
      }),
    ).rejects.toThrow("Attachments can total at most 50 MB per message.");
