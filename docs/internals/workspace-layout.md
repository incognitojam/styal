...
- `apps/desktop` (`@t3tools/desktop`): Electron shell. Supervises a desktop-scoped `t3` backend,
  loads the web bundle over the `styal://` protocol, and owns SSH-managed remote environments.
...
- `infra/relay` (`t3code-relay`): the hosted styal Link relay, deployed with Alchemy. Handles
