...
describe("PreviewManager", () => {
  ...
  effectIt.effect("bounds a stuck automation snapshot and allows a fresh attempt", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        // Each snapshot retries its stalled capture three times with a fresh capturePage call.
    ...
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        fromId.mockReturnValue({
          ...
          executeJavaScript: vi.fn(async () => ({ width: 1600, height: 900 })),
          setZoomFactor,
          isFocused: () => true,
          focus: vi.fn(),
  ...
  effectIt.effect("blocks late webview and capture starts during tab close", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        for (const exit of [registrationExit, presentationExit, recordingExit] as ReadonlyArray<
  ...
  effectIt.effect("bounds a hung native source warmup", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        yield* manager.createTab("tab_recording_hung_warmup");
        yield* manager.registerWebview("tab_recording_hung_warmup", 42);
        const startFiber = yield* manager
          .startRecording("tab_recording_hung_warmup")
          .pipe(Effect.forkChild({ startImmediately: true }));
        yield* settle(() => capturePage.mock.calls.length === 1);

        yield* TestClock.adjust(5_000);
        yield* settle(() => capturePage.mock.calls.length === 2);
        yield* TestClock.adjust(5_000);

        expect(yield* Fiber.join(startFiber)).toBeUndefined();
        expect(capturePage).toHaveBeenCalledTimes(2);
        const streams: Array<{ video?: unknown }> = [];
        host.displayMediaHandler()?.({ frame: host.mainFrame }, (value) => streams.push(value));
        expect(streams).toEqual([{ video: { routingId: 42 } }]);

        yield* manager.stopRecording("tab_recording_hung_warmup");
      }),
    ),
  );
