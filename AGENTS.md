...
The architecture of T3 Code's websocket layer (npx t3) enables a lot of awesome remote features. These have become core to the product. Whether users are connecting directly over their local network, using Tailscale, or leaning in fully with styal Link (our tunnel solution, also in this repo), we need to make sure new features are properly supported.

### 4. Multi-surface

T3 Code has 3 key app surfaces: **web**, **desktop**, and **mobile**.

**Web** is kind of two surfaces, as we have the public facing "app.styal.build" as well as locally hosting the web app through the `npx t3` command. Both need to be supported by all new features where reasonable.

**Desktop** is the main surface most users install first. It's a full Electron app that bundles the server runner as well. The desktop app can also be used as the host server, allowing remote connections from app.styal.build or the mobile app.
...
An empty database is a bad test. Seed your worktree's `.styal` with a copy of real data instead of pointing at live state:

- Copy from `~/.styal/userdata` (the developer's real data, the most realistic test set) or `~/.styal/dev`. Worktree state lives at `<worktree>/.styal/userdata`.
- Snapshot the database with `VACUUM INTO`, which is safe even while a server has the source open and yields one consistent file:

  ```bash
  mkdir -p .styal/userdata
  rm -f .styal/userdata/state.sqlite*  # VACUUM INTO refuses to overwrite
  bun -e "new (require('bun:sqlite').Database)(process.env.HOME + '/.styal/userdata/state.sqlite', { readonly: true }).run(\"VACUUM INTO '.styal/userdata/state.sqlite'\")"
  ```
...
- Do not open pull requests for routine upstream intake batches. Preserve upstream authorship, messages, and commit order on an `intake/**` branch, then use the promotion workflow to fast-forward `main` after batch validation and authorized approval. Attaching these commits to a PR creates unwanted upstream cross-references and participants; escaping only the PR body is insufficient. In separately authorized GitHub issue or PR prose, put upstream source references inside backticks and avoid account mentions. Actions summaries are not subject to that prose restriction.
...
- **Upstream is read-only and never auto-synced.** Queue → apply chronologically → validate the combined batch → promote without a PR → advance the baseline. Start with `next --count 20`, adjust the boundary for nearby reverts or corrections, and test intermediate commits only when diagnosis requires it. Follow `docs/operations/upstream-tracking.md` for the commands and current approval boundary. Never rebase `main` onto upstream or force-push it.
- Preserve upstream intake provenance in landed commit metadata. Intake commits use comma-separated `Upstream-PR: 1234, 5678` and/or `Upstream-Commit: <full lowercase SHA>` trailers. Never list a commit in `Upstream-Commit` when its PR is already in `Upstream-PR`. The promotion workflow's `source_prs` and `source_commits` inputs must exactly match that metadata so the queue can reconcile landed work. Keep fork adaptations small and validate storage isolation, installed-app identity, and affected fork capabilities rather than mechanically rebranding upstream internals. Review empty source diffs against the actual changes; never assume an adjacent commit implements that source.
- During intake, follow upstream behavior unless it conflicts with an explicitly maintained fork difference. General guidance inherited from upstream is not a separate styal policy. Record deliberate fork-specific policies and divergences in the fork feature ledger, with supporting documentation where needed, rather than inventing new differences during a backport.
