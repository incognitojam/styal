...
/**
 * Reads `STYAL_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=STYAL_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>STYAL_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=STYAL_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
function systemdManager(input: {
  ...
  return {
    ...
    restart: [
      {
        step: "reloading restored systemd user units",
        command: "systemctl",
        args: ["--user", "daemon-reload"],
        optional: true,
      },
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the styal background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "linger-disabled":
      return 'Lingering is disabled. styal will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `styal service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status styal.service`, then run `styal service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `styal service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace styal@${this.installedVersion} with older styal@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
export class BootServiceMigrationRestartRequiredError extends Schema.TaggedError<BootServiceMigrationRestartRequiredError>()(
  ...
  override get message(): string {
    return "Migrating the styal service to executable archives requires a restart. Run `styal service update` on the server machine.";
  }
}
...
export interface BootServiceStatus {
   ...
   * as its own; `styal update --base-dir` learned that by restarting the live
...
export class BootService extends Context.Service<
  BootService,
  {
    readonly install: (options?: {
       ...
       * is running now. `styal update` uses this when the user declines the
       * restart, so a later `styal service restart` lands on the new version.
       */
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const install = Effect.fn("cloud.boot_service.install")(function* (options?: {
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned styal runtime",
                  cause,
                }),
            ),
    ...
    // A protocol migration changes both files. Keep the stopped service's
    // final state so a failed activation can restore a runnable old pair.
    const previousUnit =
      installed && start
        ? yield* fs.readFileString(unitPath).pipe(Effect.option)
        : Option.none<string>();
    const previousState = installed
      ? yield* fs.readFileString(statePath).pipe(Effect.option)
      : Option.none<string>();

    yield* Effect.gen(function* () {
      if (installed && !start && Option.isNone(previousState)) {
        return yield* new BootServiceMigrationRestartRequiredError();
      }
      if (installed) {
        const previousStateText = previousState;
        if (Option.isSome(previousStateText)) {
          ...
          if (!start && parseServiceState(previousStateText.value) === undefined) {
            return yield* new BootServiceMigrationRestartRequiredError();
          }
      ...
      Effect.tapError(() =>
        installed && start
          ? Effect.gen(function* () {
              if (Option.isSome(previousState)) yield* writeDurably(statePath, previousState.value);
              if (Option.isSome(previousUnit)) yield* writeDurably(unitPath, previousUnit.value);
