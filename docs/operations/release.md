...
- Reads the shared production styal Link relay URL and Clerk client configuration before packaging clients.
...
- `APNS_ENVIRONMENT`, `APNS_TEAM_ID`, `APNS_KEY_ID`, and `APNS_BUNDLE_ID` only when enabling
  mobile push; unset, the relay deploys with push delivery disabled, and a partial group fails
  the deploy
...
2. Create an explicit App ID for `build.styal.app` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [styal Link Clerk Setup](../internals/t3-connect.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.build.styal.app` and includes
    Associated Domains.
