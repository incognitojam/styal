...
## Start one disposable styal environment
...
- Workspace: `<repo>/apps/mobile/ios/styalDev.xcworkspace`
- Scheme: `styalDev`
- Configuration: `Debug`
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in styal when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from styal Dev.
