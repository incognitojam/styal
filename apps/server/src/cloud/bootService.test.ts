...
const linuxRuntime = "/home/theo/.styal/runtime/styal-executable/versions/1.2.3/styal";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.styal",
  logPath: "/home/theo/.styal/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/styal.service",
};
...
it("reads the served T3 home back out of a rendered unit or plist", () => {
  const plan = (baseDir: string) => ({
    program: [`${baseDir}/runtime/styal-executable/versions/1.2.3/styal`, "__service-launcher"],
    baseDir,
  ...
  expect(
    BootService.bootServiceBaseDirOf(BootService.renderBootServiceUnit(plan("/home/theo/.styal"))),
  ).toBe("/home/theo/.styal");
...
const macRuntime = "/Users/theo/.styal/runtime/styal-executable/versions/1.2.3/styal";
const macPlan = {
  program: [macRuntime, "__service-launcher"],
  baseDir: "/Users/theo/.styal",
  logPath: "/Users/theo/.styal/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/build.styal.app.service.plist",
};
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.styal/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "styal-boot-service-test-" });
  const baseDir = path.join(home, ".styal");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable styal.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart styal.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop styal.service" ||
        ...
        stdout:
          input.args[0] === "--version"
            ? // The runtime under test reports the version of the directory it
              ...
              `styal v${/versions\/([^/]+)\//.exec(input.command)?.[1] ?? "1.2.3"}\n`
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          Layer.succeed(HostProcessExecutablePath, "/usr/bin/styal"),
          Layer.succeed(
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop styal.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now styal.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop styal.service",
                "systemctl --user daemon-reload",
                "systemctl --user restart styal.service",
              ]
            : [
                "launchctl bootout --wait gui/501/build.styal.app.service",
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      expect(yield* fs.readFileString(plan.unitPath)).toContain("versions/1.2.4/styal");
      expect(
        commands.filter(
      ...
      // the reason is named so `styal service status` can point at restart.
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop styal.service",
        "systemctl --user daemon-reload",
        "systemctl --user enable styal.service",
        "systemctl --user restart styal.service",
      ]);
    }),
  );
  ...
  it.effect("restart leaves a service that serves another T3 home alone", () =>
    Effect.gen(function* () {
      ...
      commands.length = 0;
      const path = yield* Path.Path;
      const otherHome = yield* fs.makeTempDirectoryScoped({ prefix: "styal-other-home-" });

      const other = yield* makeService(undefined, "1.2.3", path.join(otherHome, ".styal"));
  ...
  it.effect.each([
    ...
    "systemctl --user enable styal.service",
    "systemctl --user restart styal.service",
  ])("migrates protocol 3 without removing npm runtimes (failure: %s)", (fail) =>
    Effect.gen(function* () {
      const { service, fs, statePath, control, runtime, commands } = yield* makeHarness();
      const plan = yield* service.install();
      const legacyState = '{"protocol":3,"activeVersion":"1.2.3"}';
      const legacyUnit = `Environment=STYAL_HOME=${plan.baseDir}\nExecStart=/usr/bin/node ${plan.baseDir}/runtime/service-launcher.mjs\n`;
      const legacyRuntime = `${plan.baseDir}/runtime/styal-cli/versions/1.2.3/node_modules/@styal/cli/dist/bin.mjs`;
      const path = yield* Path.Path;
      yield* fs.makeDirectory(path.dirname(legacyRuntime), { recursive: true });
      yield* fs.writeFileString(legacyRuntime, "// old npm runtime\n");
      yield* fs.writeFileString(statePath, legacyState);
      yield* fs.writeFileString(plan.unitPath, legacyUnit);
      expect((yield* service.install({ start: false }).pipe(Effect.flip))._tag).toBe(
        "BootServiceMigrationRestartRequiredError",
      );
      expect(yield* fs.readFileString(statePath)).toBe(legacyState);
      if (fail) {
        control.failCommand = fail;
        ...
        expect(yield* fs.readFileString(statePath)).toBe(legacyState);
        expect(yield* fs.readFileString(plan.unitPath)).toBe(legacyUnit);
      } else {
        yield* service.install();
        expect(parseServiceState(yield* fs.readFileString(statePath))?.protocol).toBe(4);
        expect(yield* fs.readFileString(plan.unitPath)).toContain(
          `${runtime.entryPath} __service-launcher`,
        );
      }
      expect(yield* fs.readFileString(legacyRuntime)).toBe("// old npm runtime\n");
    }),
  );

  it.effect("requires a restart when an installed service has no state file", () =>
    Effect.gen(function* () {
      const { service, fs, statePath } = yield* makeHarness();
      const plan = yield* service.install();
      const previousUnit = yield* fs.readFileString(plan.unitPath);
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "build.styal.app.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/build.styal.app.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/build.styal.app.service",
        "launchctl enable gui/501/build.styal.app.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/build.styal.app.service";
