...
Shift keys together. styal attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while styal is in front captures styal itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. styal refuses shortcuts
...
terminal contents. If an app is slow to answer, styal attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **styal SnapShots** extension during setup. It is installed
...
shortcut does not fire, check styal under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. styal shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. styal shows the exact binding it will
...
**Other Wayland desktops.** styal uses the desktop's screenshot portal when it can capture the
