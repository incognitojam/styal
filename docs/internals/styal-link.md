...
When the repository setup action is saved for a checkout, it symlinks
`$STYAL_PROJECT_ROOT/.env` (and `infra/relay/.env`) into each worktree, so populate the file once in
the original checkout rather than replacing the symlink.
