...
const PersistedComposerImageAttachmentSchema = Schema.Struct({
...
const LegacyPersistedComposerFileAttachment = Schema.Struct({
...
export const PersistedComposerImageAttachment = PersistedComposerImageAttachmentSchema;
export type PersistedComposerImageAttachment = typeof PersistedComposerImageAttachment.Type;
export const PersistedComposerAttachment = Schema.Union([
  PersistedComposerImageAttachmentSchema,
  LegacyPersistedComposerFileAttachment,
]);
...
const PersistedIssueContextDraft = Schema.Struct({
  id: Schema.String,
  threadId: ThreadId,
  ...
  author: Schema.NullOr(Schema.String),
  body: Schema.String,
  comments: Schema.Array(PersistedIssueContextComment),
});
type PersistedIssueContextDraft = typeof PersistedIssueContextDraft.Type;
...
interface ComposerDraftStoreState {
   ...
   */
  addIssueContext: (threadRef: ComposerThreadTarget, selection: IssueContextSelection) => boolean;
  /**
   * Replace the entire issue-contexts list (used by send-failure retry to
   * restore the pre-send snapshot).
   */
  setIssueContexts: (
    threadRef: ComposerThreadTarget,
  ...
  removeIssueContext: (threadRef: ComposerThreadTarget, contextId: string) => void;
  clearIssueContexts: (threadRef: ComposerThreadTarget) => void;
...
function revokeDraftThreadPreviewUrls(draft: ComposerThreadDraftState | undefined): void {
  ...
  for (const image of draft.images) {
    if (image.type === "image") revokeObjectPreviewUrl(image.previewUrl);
  }
}
...
function normalizePersistedIssueContextDraft(
  ...
  if (
    ...
    attachedAt.length === 0 ||
    !isSourceControlProviderKind(provider) ||
    typeof title !== "string" ||
    title.length === 0 ||
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadKey, draft] of Object.entries(state.draftsByThreadKey)) {
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      prompt: draft.prompt,
      // Legacy bytes remain in storage until this same atomic write saves an upload reference.
      attachments: draft.persistedAttachments.filter(
        (attachment) =>
          attachment.type !== "file" ||
          !draft.files.some(
            (file) =>
              file.id === attachment.id && file.uploadedAttachmentId && file.uploadEnvironmentId,
          ),
      ),
      ...(draft.files.length > 0
        ? {
      ...
      ...(draft.issueContexts.length > 0
        ? {
            issueContexts: draft.issueContexts.map((context) => ({
              id: context.id,
              threadId: context.threadId,
    ...
    const nextDraft: ComposerThreadDraftState = {
      ...
      persistedAttachments: [
        ...
        ...current.persistedAttachments.filter(
          (attachment) =>
            attachment.type === "file" && current.files.some((file) => file.id === attachment.id),
        ),
      ],
      nonPersistedImageIds,
    };
...
export function hydrateImagesFromPersisted(
  ...
  return hydrateAttachmentsFromPersisted(attachments).filter(
  ...
  return {
    ...
    issueContexts:
      persistedDraft.issueContexts?.map((context) => ({
        ...context,
        comments: context.comments.map((comment) => ({ ...comment })),
      })) ?? [],
    previewAnnotations:
      ...
      return {
        ...
        addImages: (threadRef, images, limits) => {
          ...
          set((state) => {
            ...
            const existingIds = new Set(
              [...existing.images, ...existing.files].map((image) => image.id),
            );
            const existingDedupKeys = new Set(
              [...existing.images, ...existing.files].map((image) =>
                composerAttachmentDedupKey(image),
              ),
            );
            ...
            let totalBytes = [...existing.images, ...existing.files].reduce(
        ...
        addFiles: (threadRef, files) => {
          ...
          set((state) => {
            ...
            const rejectedByCount: ComposerFileAttachment[] = [];
            const rejectedByTotalBytes: ComposerFileAttachment[] = [];
            let totalBytes = [...existing.images, ...existing.files].reduce(
              (total, attachment) => total + attachment.sizeBytes,
              0,
            );
            ...
            for (const file of files) {
              ...
              if (totalBytes + file.sizeBytes > PROVIDER_SEND_TURN_MAX_TOTAL_ATTACHMENT_BYTES) {
                rejectedByTotalBytes.push(file);
                continue;
              }
              accepted.push(file);
        ...
        addIssueContext: (threadRef, selection) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef);
          const threadId = resolveComposerThreadId(get(), threadRef);
          if (!threadKey || !threadId) return false;
          let accepted = false;
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const dedupKey = issueContextDedupKey(selection);
            if (existing.issueContexts.some((entry) => issueContextDedupKey(entry) === dedupKey)) {
              return state;
            }
            accepted = true;
            const draft: IssueContextDraft = {
              ...selection,
              id: newIssueContextId(),
              threadId,
            ...
            return {
              draftsByThreadKey: {
                ...
                [threadKey]: {
                  ...existing,
                  issueContexts: [...existing.issueContexts, draft],
                },
              },
            };
          });
        ...
        removeIssueContext: (threadRef, contextId) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0 || contextId.length === 0) return;
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current) return state;
            const filtered = current.issueContexts.filter((entry) => entry.id !== contextId);
            if (filtered.length === current.issueContexts.length) return state;
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              issueContexts: filtered,
            };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
        ...
        clearIssueContexts: (threadRef) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef) ?? "";
          if (threadKey.length === 0) return;
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current || current.issueContexts.length === 0) return state;
            const nextDraft: ComposerThreadDraftState = {
              ...current,
              issueContexts: [],
            };
            ...
            const nextDraft: ComposerThreadDraftState = {
              ...
              persistedAttachments: [
                ...current.persistedAttachments.filter(
                  (attachment) =>
                    attachment.type === "file" &&
                    !attachments.some((next) => next.id === attachment.id),
                ),
                ...attachments,
              ],
              nonPersistedImageIds: current.nonPersistedImageIds.filter(
...
/** Confirms the current draft snapshot reached browser storage before another owner releases it. */
export function persistComposerDraftsNow(): boolean {
  ...
  try {
    ...
    return localStorage.getItem(COMPOSER_DRAFT_STORAGE_KEY) === expected;
  } catch {
    return false;
  }
}
