// This file mostly exists because we want dev mode to say "styal (Dev)" instead of "electron"
...
export function makeDevelopmentEnvironmentScript(environment) {
  const envEntries = [
    ...
    ["STYAL_HOME", environment.STYAL_HOME],
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSScreenCaptureUsageDescription:
      "styal captures the active window when you use the snapshot shortcut.",
    NSDocumentsFolderUsageDescription: "styal reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the styal name.
