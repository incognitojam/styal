...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("review diff previews", () => {
    ...
    it.effect("reports diff paths marked generated by Git attributes", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        yield* writeTextFile(
          cwd,
          ".gitattributes",
          "packages/app/café.lock linguist-generated=true\n",
        );
        yield* writeTextFile(cwd, "packages/app/café.lock", "generated contents\n");
        yield* writeTextFile(cwd, "notes.txt", "review me\n");

        const preview = yield* driver.getReviewDiffPreview({
          cwd: pathService.join(cwd, "packages/app"),
        });
        const workingTree = preview.sources.find((source) => source.kind === "working-tree");

        assert.deepStrictEqual(workingTree?.generatedPaths, ["packages/app/café.lock"]);
        assert.include(workingTree?.diff, "+++ b/packages/app/café.lock");
      }),
    );
