...
describe("video asset byte ranges", () => {
  ...
  it.effect(
    "honors matching If-Range validators and uses full responses for HEAD or mismatches",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        ...
        yield* fs.writeFileString(file, "0123456789");
        const asset = { path: file, mimeType: "video/mp4" };
        const full = HttpServerResponse.toWeb(yield* assetFileResponse(asset));
        ...
        for (const validator of [etag, modified]) {
          const partial = HttpServerResponse.toWeb(
            yield* assetFileResponse(asset, "bytes=4-6", validator),
          );
          expect(partial.status).toBe(206);
          expect(partial.headers.get("etag")).toBe(etag);
          expect(partial.headers.get("content-range")).toBe("bytes 4-6/10");
          expect(yield* Effect.promise(() => partial.text())).toBe("456");
        }
        for (const ifRange of [
          ...
          const response = HttpServerResponse.toWeb(
            yield* assetFileResponse({ path: file, mimeType: "video/mp4" }, "bytes=0-1", ifRange),
          );
          expect(response.status).toBe(200);
          expect(response.headers.has("content-range")).toBe(false);
          expect(yield* Effect.promise(() => response.text())).toBe("0123456789");
        }
        const head = yield* assetFileResponse(
          { path: file, mimeType: "video/mp4" },
          "bytes=0-1",
          undefined,
          "HEAD",
        );
        ...
        expect(HttpServerResponse.toWeb(head).headers.get("content-length")).toBe("10");
      }).pipe(Effect.provide(fileResponseLayer)),
  );
