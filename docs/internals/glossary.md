...
| Default repository | The repository a checkout with several remotes targets, stored as `remote.<name>.gh-resolved` in its Git config (the key `gh repo set-default` writes). It sets the checkout's identity and sidebar grouping only while the current branch has no upstream. |
