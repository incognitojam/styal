...
>()("@styal/cli/pullRequest/PullRequestService") {}
...
function withRateLimitBackoff(
  ...
  return {
    ...
    ...(api.getFile === undefined ? {} : { getFile: wrap("getFile", api.getFile) }),
...
export const make = Effect.gen(function* () {
  ...
  const attachGeneratedPaths = (cwd: string, slice: PullRequestDiffResult) => {
    ...
    return checkpointStore.readGeneratedDiffPaths({ cwd, paths }).pipe(
      ...
      Effect.map((generatedPaths): PullRequestDiffResult =>
  ...
  const refEpoch = (scope: RefCacheScope, ref: PullRequestRef) =>
    Math.max(turnRefreshEpoch, refEpochs.get(refScope(ref))?.[scope] ?? 0);
  const refCacheKey = (ref: PullRequestRef) =>
    JSON.stringify([refEpoch("detail", ref), ref.projectId, ref.repository, ref.number]);
  ...
  const statsBatchKey = (refs: Iterable<PullRequestRef>) =>
    JSON.stringify([
      listingsEpoch,
      [...refs]
        .map((ref) => [ref.projectId, ref.repository, ref.number, refEpoch("detail", ref)] as const)
        .toSorted((left, right) =>
  ...
  const invalidate: PullRequestService["Service"]["invalidate"] = (input) => {
    ...
    if (reference !== undefined) {
      return Effect.sync(() => bumpRefEpoch(reference, input.scope));
    }
    return Effect.sync(() => {
