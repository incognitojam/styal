...
/**
 * Inbox sort: static lifecycle order, newest anchor on top, with same-workspace
 * threads clustered — the original thread first, spawned follow-ups beneath
 * it in creation order. A cluster sits at its newest member's creation or
 * re-entry anchor: ordinary activity never reorders the list, while spawning
 * a sibling or un-settling one surfaces the family without scrambling it.
 * Shared by web and mobile so both render identical inbox orders.
 */
