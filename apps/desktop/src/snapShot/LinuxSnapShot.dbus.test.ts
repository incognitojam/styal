...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    expect(await portal.backend("build.styal.Styal")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("build.styal.Styal")).toBe("gnome-extension");
    expect(await extension.captureExtension("build.styal.Styal")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["build.styal.Styal.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("build.styal.Styal")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("build.styal.Styal", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("styal");
    ...
    expect(activateTitle).toBe("styal");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("build.styal.Styal")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "build.styal.Styal.NiriTest.SnapShot",
      "--object-path",
      "/build/styal/SnapShot",
      "--method",
      "build.styal.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "build.styal.Styal.NiriTest.SnapShot",
        path: "/build/styal/SnapShot",
        interface: "build.styal.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
