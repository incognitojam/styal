...
## Using It with styal Connect

styal Connect may offer to install the service during setup so the host stays reachable in the
background. This is only an onboarding shortcut: the service and styal Connect are managed separately.

Signing out of styal Connect does not remove the service. Use `t3 service uninstall` when you no longer
