 ...
 * behind `npx @styal/cli` / `npm i -g @styal/cli`: one `@styal/cli-<platformKey>` package per
 * archive holding the archive's contents verbatim, plus the `@styal/cli` launcher
 ...
 *
 *   @styal/cli-<platformKey>/      archive contents flattened + package.json
 *   @styal/cli-<platformKey>.tgz   the same tree as an npm tarball
 *   @styal/cli/                         launcher: package.json, bin/styal.js, README.md
 *   @styal/cli.tgz                      the launcher as an npm tarball
 *
...
export const NPM_PLATFORM_PACKAGE_SCOPE = "@styal";
export const NPM_LAUNCHER_PACKAGE_NAME = "@styal/cli";
 ...
 * four. The archive's runtime `node_modules` (native addons and their
...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `styal CLI executable for ${platformKey}`,
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    os: [os],
    cpu: [cpu],
    files: ["styal", "styal.exe", "client", "resource-monitor", "node_modules"],
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The styal CLI executable for ${platformKey}. Do not install this package directly:`,
    ...
    "```",
    "",
    "Source and documentation: https://github.com/incognitojam/styal",
    "",
  ].join("\n");
}

/** package.json for the `styal` launcher. No engines: bin/styal.js is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "styal CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { styal: "./bin/styal.js" },
    files: ["bin", "dist"],
...
/**
 * The launcher every `npx @styal/cli` runs. Plain CommonJS with no dependencies so it
...
try {
  ...
  process.stderr.write(
    [
      "styal: no styal CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall styal so npm fetches its optional dependency.",
      "The desktop app and release archives are at https://github.com/incognitojam/styal/releases",
      "",
    ].join("\\n"),
  );
  process.exit(1);
...
const executable = join(packageDir, process.platform === "win32" ? "styal.exe" : "styal");
const child = spawn(executable, process.argv.slice(2), { stdio: "inherit" });
// Keep the npm launcher alive until the server drains and releases its port.
for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"]) {
  process.on(signal, () => child.kill(signal));
}
child.on("error", (error) => {
  process.stderr.write("styal: failed to start " + executable + ": " + error.message + "\\n");
  process.exit(1);
});
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = input.key.startsWith("win32") ? "styal.exe" : "styal";
...
/** Writes the launcher package (package.json, bin/styal.js, README) and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, "bin/styal.js");
  ...
  yield* fs.copyFile(license, path.join(stageDir, "LICENSE"));
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.string("archives-dir").pipe(
      Flag.withDescription("Directory holding the release's styal-<version>-<platform> archives."),
    ),
  ...
  Command.withDescription(
    "Build the styal launcher and @styal/cli-<platform> npm packages from CLI release archives.",
  ),
);
