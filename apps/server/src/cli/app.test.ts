...
describe("styal app", () => {
    ...
    withTempDirectory("t3-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`styal app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
    ...
    withTempDirectory("t3-app-platform-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`styal app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
    ...
    withTempDirectory("t3-app-missing-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the styal desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses STYAL_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("t3-app-command-test-", (root) =>
      Effect.gen(function* () {
        ...
        yield* runCli(["app"], { STYAL_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("t3-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        ...
        const baseDir = NodePath.join(root, ".styal");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("never searches a dev state directory for an explicit T3 home", () =>
    withTempDirectory("t3-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        ...
        const envError = yield* runCli(["app"], { STYAL_HOME: baseDir }).pipe(Effect.flip);
