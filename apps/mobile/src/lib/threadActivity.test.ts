...
describe("buildThreadFeed", () => {
  ...
  it("keeps OpenCode detail-only output when it equals the command", () => {
    ...
    expect(workEntryRowLabel(row!, true)).toBe("Command");
  });
  ...
  it.each([
    ...
    if (group?.type !== "activity-group") return;
    const [row] = group.activities;
    expect(workEntryRowLabel(row!)).toBe(input.label);
  ...
  it.each(["runtime.error", "runtime.warning"] as const)(
    ...
    (kind) => {
      ...
      expect(row.canExpand).toBe(true);
      expect(workEntryRowLabel(row, true)).toBe(message);
  ...
  it.each([
    ...
    const row = group.activities[0]!;
    expect(row.canExpand).toBe(true);
    expect(workEntryRowLabel(row)).toBe(input.detail.replace(/\s+/g, " "));
    expect(workEntryRowLabel(row, true)).toBe(input.detail);
    expect(row.getFullDetail()).toBeNull();
  ...
  it("drops a generated worktree name from edited-file paths", () => {
    ...
    expect(workEntryRowLabel(group.activities[0]!, true)).toBe(
  ...
  it("keeps viewed image metadata while collapsing a streamed Claude Read", () => {
    ...
    expect(row.canExpand).toBe(true);
    expect(row.getFullDetail()).toBeNull();
    expect(workEntryRowLabel(row, true)).toBe(`${imagePath.slice(0, 177)}...`);
  });
  ...
  it("keeps MCP inputs available to expanded mobile work rows", () => {
    ...
    expect(workEntryRowLabel(group.activities[0]!, true)).toBe("repository.search");
