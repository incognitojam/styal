...
// self-contained `styal` executable (Node inside); the mounted server tree is a
    ...
    if (runtime.ok) {
      ...
      if (stagedProbe.ok) {
        ...
        return {
          ...
          runtime: { kind: "executable", entryPath: `${runtime.linuxAppRoot}/styal` },
...
> {
  ...
  // Build an explicit copy of process.env minus STYAL_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.styal via
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus STYAL_HOME; pass it
