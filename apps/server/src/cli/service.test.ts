...
const status = {
  ...
  logPath: "/home/me/.styal/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "styal service",
      "  Status: installed · styal@0.0.29",
      "  Unit: /home/me/.config/systemd/user/styal.service",
      "  Logs: /home/me/.styal/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `styal service install` to repair it.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("Run `styal service install` to repair it.");
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(output, "styal@0.0.32-nightly.1 (newer than this styal@0.0.31 CLI)");
  assert.include(output, "Run `styal update 0.0.32-nightly.1` to match it");
  ...
  const service = BootService.BootService.of({
    ...
    install: (options) =>
      Effect.sync(() => {
        ...
        return {
          program: [
            "/test/styal/runtime/styal-executable/versions/1.0.0/styal",
            "__service-launcher",
          ],
          baseDir: "/test/styal",
...
it.layer(Layer.mergeAll(NodeServices.layer, NetService.layer))("service commands", (it) => {
  it.effect("restart restarts the installed service", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "styal-service-cli-test-" });
