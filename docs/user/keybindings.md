...
`~/.styal/userdata/keybindings.json` by default. You can edit this file directly.
...
styal creates the file with its defaults and adds new defaults on later startups.
...
Invalid rules are ignored; if the file cannot be parsed, styal uses defaults.
...
Available context keys are `terminalFocus`, `terminalOpen`, `rightPanelFocus`,
`previewFocus`, `previewOpen`, and `modelPickerOpen`. Unknown keys evaluate to
`false`.
...
`sourceControl.focusPullRequestTab` (`mod+shift+g`) opens the active thread's
open pull request in the right panel.
