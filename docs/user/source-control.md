...
When a GitHub project has multiple remotes, the **Pull requests** page follows the repository
selected by `gh repo set-default`. Without an explicit selection, it follows the current branch's
tracked remote. The current branch's PR indicator also follows its tracked remote, so a fork
checkout does not pick up a same-named branch from `upstream`.
