...
describe.skipIf(HostProcessPlatform.defaultValue() !== "linux")("installer terminal", () => {
  it.each([false, true])(
    ...
    async (fail) => {
      const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "styal-install-progress-"));
      const version = "1.2.3";
      const stem = `styal-${version}-linux-${HostProcessArchitecture.defaultValue()}`;
      ...
      const child = NodeChildProcess.spawn("script", ["-qec", `sh '${installer}'`, "/dev/null"], {
        env: {
          ...
          TERM: "xterm",
          NO_COLOR: "1",
          STYAL_VERSION: version,
          STYAL_HOME: NodePath.join(root, "home"),
          STYAL_INSTALL_BIN_DIR: NodePath.join(root, "bin"),
          STYAL_RELEASE_BASE_URL: `http://127.0.0.1:${address.port}`,
        },
      ...
      try {
        ...
        const versions = NodePath.join(root, "home/runtime/styal-executable/versions");
        if (fail) {
          expect(code).not.toBe(0);
          expect(output).toContain("500");
          expect(output).not.toContain("100%");
          expect(output).not.toContain("Installed styal");
          ...
          expect(output).toContain("Installed styal 1.2.3");
          expect(
            await NodeFSP.readFile(NodePath.join(versions, version, ".install-complete"), "utf8"),
          ).toBe("1.2.3\n");
          expect(
            NodeChildProcess.execFileSync(NodePath.join(root, "bin/styal"), ["--version"], {
              encoding: "utf8",
            }).trim(),
          ).toBe("styal v1.2.3");
