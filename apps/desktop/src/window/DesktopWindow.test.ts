...
describe("DesktopWindow", () => {
  ...
  it.effect("does not focus preview webviews when they navigate", () =>
    Effect.gen(function* () {
      const fakeWindow = makeFakeBrowserWindow();
      const createCount = yield* Ref.make(0);
      const mainWindow = yield* Ref.make<Option.Option<Electron.BrowserWindow>>(Option.none());
      ...
      yield* Effect.gen(function* () {
        ...
        const webPreferences = {} as Electron.WebPreferences;
        willAttachWebview({ preventDefault: vi.fn() }, webPreferences, {
          partition: "persist:t3code-preview-test",
        });

        assert.isFalse(webPreferences.focusOnNavigation);
      }).pipe(Effect.provide(layer));
    }),
  );
