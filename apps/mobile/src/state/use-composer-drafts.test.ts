...
describe("mobile composer drafts", () => {
  ...
  it("retains explicit model-only drafts and share receipts across the cloud-store upgrade", async () => {
    vi.useFakeTimers();
    const drafts = {
      "new-task:environment-1:project-1": {
        text: "",
      ...
      "environment-1:shared": { text: "", attachments: [], importedShareIds: ["share-1"] },
    };
    composerDraftFileMocks.setDocument({ schemaVersion: 1, drafts });
    await waitForComposerDraftsLoaded();
    await restoreCloudComposerDrafts("account-1");
    const persisted: Record<string, ComposerDraft> = JSON.parse(
      composerDraftFileMocks.getDocument(),
    ).drafts;
    expect(persisted["environment-1:shared"]).toEqual(drafts["environment-1:shared"]);
    // The legacy new-task key moves to an id key stamped with its project.
    const newTaskDrafts = Object.entries(persisted).filter(([key]) => key.startsWith("new-task:"));
    expect(newTaskDrafts).toHaveLength(1);
    expect(newTaskDrafts[0]?.[1]).toMatchObject({
      modelSelection: { instanceId: "codex", model: "gpt-5.4" },
    ...
    appAtomRegistry.set(composerDraftsAtom, {});
    await waitForComposerDraftsLoaded();
    expect(appAtomRegistry.get(composerDraftsAtom)).toEqual(persisted);
  });
  ...
  it("blocks overwrite and account activation when an archived queue cannot be decoded", async () => {
    ...
    composerDraftFileMocks.setDocument({
      ...
      signedOutDrafts: {
        "account-1": { drafts: {}, queuedMessages: [{ messageId: "incomplete-message" }] },
      },
    });
    const original = composerDraftFileMocks.getDocument();
    await expect(restoreCloudComposerDrafts("account-2")).rejects.toMatchObject({
      operation: "decode",
    });
    setComposerDraftText("direct:thread", "Keep this new edit");
    await expect(flushComposerDrafts()).rejects.toMatchObject({ operation: "decode" });
    expect(composerDraftFileMocks.getDocument()).toBe(original);
    expect(appAtomRegistry.get(composerCloudDraftsAtom).accountId).toBeNull();
  });
  ...
  it("keeps share-import receipts on otherwise contentless new-task drafts", () => {
    ...
    // account archive and restore. Legacy keys move to an id key stamped with
    // their project.
    const modelSelection = {
      instanceId: ProviderInstanceId.make("codex"),
      model: "gpt-5.4",
    };
    const decoded = Object.values(
      decodePersistedComposerState({
        schemaVersion: 1,
        drafts: {
          "new-task:environment-1:project-1": {
  ...
  it("persists an async merge rollback with the draft model selection", async () => {
    const draftKey = "environment-1:thread-1";
    const modelSelection = { instanceId: ProviderInstanceId.make("codex"), model: "gpt-5.6-sol" };
    const snapshot: ComposerDraft = { text: "typed before", attachments: [], modelSelection };
    const merged: ComposerDraft = {
      text: "typed before\n\nqueued text",
      attachments: [],
      modelSelection,
