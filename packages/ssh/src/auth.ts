...
const SSH_ASKPASS_DIR_NAME = "styal-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when styal re-runs ssh with a cached password
...
printf 'styal ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when styal re-runs\r
...
[Console]::Error.WriteLine("styal ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "styal-ssh-runtime-" });
