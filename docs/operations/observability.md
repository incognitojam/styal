...
stdout/stderr at `~/.styal/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.styal/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.styal/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.styal/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.styal/dev/logs/server.trace.ndjson`.
...
```bash
TRACE_FILE="${STYAL_HOME:-$HOME/.styal}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.styal/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.styal/dev/logs/server.trace.ndjson"
```
