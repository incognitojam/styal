...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/incognitojam/styal/releases/download'",
    );
    assert.include(
      script,
      'T3_RUNTIME_DIR="$HOME/.styal/runtime/styal-executable/versions/$T3_ARCHIVE_VERSION"',
    );
    assert.include(script, 'T3_ARCHIVE="styal-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"');
    assert.include(script, "SHA256SUMS");
    assert.include(script, 'exec "$T3_RUNTIME_DIR/styal" "$@"');
    assert.notInclude(script, "npx");
    assert.notInclude(script, "npm exec");
    assert.notInclude(script, "styal@latest");
    assert.notInclude(script, 'exec styal "$@"');
    ...
    assert.isBelow(
      script.indexOf('"$T3_STAGING/styal" --version'),
    ...
    const launch = buildRemoteLaunchScript({
      ...ARCHIVE,
      releaseBaseUrl: "https://mirror.example/styal/",
    });
    assert.include(launch, "T3_ARCHIVE_MODE=1");
    assert.include(launch, "T3_RELEASE_BASE_URL='https://mirror.example/styal'");
  ...
  it("builds the remote styal runner with a node script override", () => {
  ...
  it("uses the remote styal runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "styal-archive-runner-" });
        const releaseBaseUrl = yield* makeMirror(root);
        const runner = `${root}/run-styal.sh`;
        ...
        for (const result of results) {
          ...
          assert.include(result.stdout, `styal v${archiveVersion}`);
        }
        const versionsDir = `${home}/.styal/runtime/styal-executable/versions`;
