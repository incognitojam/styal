...
If reordering is unavailable for one environment, update the styal server running in that
...
pill** fallback because their colors are not controlled by styal.

To generate a fresh title from the conversation, open a thread's context menu and choose
**Regenerate title**. While styal is generating it, the action reads **Regenerating…** and cannot
