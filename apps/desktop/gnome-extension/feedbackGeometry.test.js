...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "styal");
  expect(findCaptureDestination([window(99, "styal"), target], 42, "styal")).toBe(target);
  expect(findCaptureDestination([window(99, "styal")], 42, "styal")).toBeUndefined();
