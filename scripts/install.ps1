# Installs the styal CLI from a GitHub Release archive on Windows. Needs
...
#
# Environment:
#   STYAL_CHANNEL           release train to follow: stable, nightly, or preview
#                            (default: stable; preview is a maintainers' test train)
#   STYAL_VERSION           exact version to install (overrides STYAL_CHANNEL)
#   STYAL_HOME              T3 home directory (default: ~\.styal)
#   STYAL_INSTALL_BIN_DIR   where styal.exe is linked (default: ~\.local\bin)
#   STYAL_RELEASE_BASE_URL  mirror for releases/download (default: GitHub)
#
# The archive is unpacked into $STYAL_HOME\runtime\styal-executable\versions\<version>, the
# same layout `styal service install` uses, so the service reuses this download.
$ErrorActionPreference = "Stop"
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

$repo = "incognitojam/styal"
$baseUrl = if ($env:STYAL_RELEASE_BASE_URL) { $env:STYAL_RELEASE_BASE_URL.TrimEnd("/") } else { "https://github.com/$repo/releases/download" }
$styalHome = if ($env:STYAL_HOME) { $env:STYAL_HOME } else { Join-Path $HOME ".styal" }
$binDir = if ($env:STYAL_INSTALL_BIN_DIR) { $env:STYAL_INSTALL_BIN_DIR } else { Join-Path $HOME ".local\bin" }

function Fail([string] $message) {
  Write-Error "styal install: $message"
  exit 1
}
...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}styal$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
$channel = if ($env:STYAL_CHANNEL) { $env:STYAL_CHANNEL } else { "stable" }
$version = $env:STYAL_VERSION
if (-not $version) {
  ...
  $tagPattern = switch ($channel) {
    ...
    default { Fail "STYAL_CHANNEL must be stable, nightly, or preview" }
  }
  $releases = Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/releases?per_page=100" -Headers @{ "User-Agent" = "styal-install" }
  $tag = ($releases | Where-Object { -not $_.draft -and $_.tag_name -match $tagPattern } | Select-Object -First 1).tag_name
  if (-not $tag) { Fail "could not find a $channel release; set STYAL_VERSION" }
  $version = $tag.Substring(1)
}
if ($version -match '-preview\.') {
  Write-Warning "styal $version is a preview build. Preview builds are cut by maintainers from unreleased branches to exercise the release pipeline. They can be broken, receive no fixes, and are never offered as updates. Set STYAL_CHANNEL=stable (the default) for a supported build."
  if ($channel -ne "preview" -and -not $env:STYAL_VERSION) {
    Fail "refusing a preview build that was not explicitly requested"
  }
}

$stem = "styal-$version-win32-$arch"
$archive = "$stem.zip"
$versionsDir = Join-Path $styalHome "runtime\styal-executable\versions"
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    [Console]::Error.WriteLine("  ${muted}Installing$reset styal $bold$version$reset`n")
    Step "Downloading..."
    try {
      ...
      if ($status -eq 404) {
        Fail "styal $version has no release archive for win32-$arch; releases before the self-contained CLI can only be installed with 'npm install -g @styal/cli@$version'"
      }
      throw
    }
    ...
    & (Join-Path $staging "styal.exe") --version | Out-Null
...
Step "Setting up the styal command..."
New-Item -ItemType Directory -Force -Path $binDir | Out-Null
$shim = Join-Path $binDir "styal.cmd"
# UTF-8 without a BOM: cmd.exe reads the shim as-is, and ASCII would corrupt
# non-ASCII characters in the user's home path.
[System.IO.File]::WriteAllText($shim, "@echo off`r`n`"$(Join-Path $targetDir 'styal.exe')`" %*", (New-Object System.Text.UTF8Encoding $false))
if ($interactive) { [Console]::Error.Write("`r$esc[2K") }
[Console]::Error.WriteLine("  ${green}Installed styal $version$reset`n")
if (($env:PATH -split ";") -notcontains $binDir) {
  Write-Host "  Add $binDir to your PATH, then run ${bold}styal$reset.`n"
} else {
  Write-Host "  Run ${bold}styal$reset to get started.`n"
}
