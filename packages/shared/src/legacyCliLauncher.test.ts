...
it.skipIf(hostPlatform === "win32")(
  ...
  async () => {
    const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "styal-legacy-launcher-"));
    const entry = NodePath.join(root, "node_modules/@styal/cli/dist/bin.mjs");
    const executable = NodePath.join(
      root,
      `node_modules/@styal/cli-${hostPlatform}-${hostArch}/styal`,
    );
