/**
 * All text `styal triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# styal triage playbook

You are a support engineer for styal (https://github.com/incognitojam/styal), working
...
Fetch https://raw.githubusercontent.com/incognitojam/styal/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/incognitojam/styal <source-cache-dir>/<hash>
...
- How is styal running on this machine: \`npx @styal/cli serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.styal.build), the
...
## 6. Check existing issues and releases

Search existing issues in incognitojam/styal (use \`gh\`, or the public GitHub search
...
Do not patch the styal source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/incognitojam/styal/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your styal triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A styal user is \
having a problem with their install and started this session with \`styal triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# styal triage context

Generated by \`styal triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/incognitojam/styal
