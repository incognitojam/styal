...
if [ -z "$T3_ARCHIVE_VERSION" ]; then
  printf 'No styal release version was provided for the remote runtime.\\n' >&2
  exit 1
fi
# Self-contained release archive: no Node, npm, or compiler on the remote.
# Unpacked into the pinned-runtime layout so \`styal service install\` reuses it.
T3_RELEASE_BASE_URL=@@T3_RELEASE_BASE_URL@@
T3_RUNTIME_DIR="$HOME/.styal/runtime/styal-executable/versions/$T3_ARCHIVE_VERSION"
t3_runtime_ready() {
  [ -x "$T3_RUNTIME_DIR/styal" ] && [ "$(cat "$T3_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$T3_ARCHIVE_VERSION" ]
}
if ! t3_runtime_ready; then
  mkdir -p "$HOME/.styal/runtime/styal-executable/versions"
  ...
  # under it.
  T3_LOCK="$HOME/.styal/runtime/styal-executable/versions/.$T3_ARCHIVE_VERSION.install.lock"
  ...
  while ! mkdir "$T3_LOCK" 2>/dev/null; do
    ...
    if [ "$T3_LOCK_WAITED" -ge @@T3_ARCHIVE_LOCK_WAIT_SECONDS@@ ]; then
      printf 'Another styal %s installation has held %s for too long.\\n' "$T3_ARCHIVE_VERSION" "$T3_LOCK" >&2
      exit 1
    fi
    sleep 1
...
if ! t3_runtime_ready; then
  case "$(uname -s)" in
    ...
    *) printf 'Remote host %s has no styal release archive.\\n' "$(uname -s)" >&2; exit 1 ;;
  esac
  case "$(uname -m)" in
  ...
  T3_ARCHIVE="styal-$T3_ARCHIVE_VERSION-$T3_PLATFORM-$T3_ARCH.tar.gz"
  T3_STAGING="$(mktemp -d "$HOME/.styal/runtime/styal-executable/versions/.staging-XXXXXX")"
  ...
  if ! "$T3_STAGING/styal" --version >/dev/null 2>&1; then
    printf 'The styal %s executable does not run on this host.\\n' "$T3_ARCHIVE_VERSION" >&2; exit 1
  fi
...
exec "$T3_RUNTIME_DIR/styal" "$@"
`;

const REMOTE_LAUNCH_SCRIPT = `set -eu
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.styal/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.styal"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-styal.sh"
RUNNER_NEXT="$STATE_DIR/run-styal.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
STATE_DIR="$HOME/.styal/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.styal"
RUNNER_FILE="$STATE_DIR/run-styal.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export class SshInvalidArchiveVersionError extends Schema.TaggedErrorClass<SshInvalidArchiveVersionError>()(
  ...
  override get message(): string {
    return `'${this.archiveVersion}' is not an exact styal version and cannot name a runtime directory.`;
  }
}
...
export class SshMissingRunnerError extends Schema.TaggedErrorClass<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote styal runner needs an archive version or a node script path.";
  }
}
