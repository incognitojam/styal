 ...
 * archive: the `styal` binary, the web client, the resource monitor, and a
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `styal-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  const libraries = (yield* fs.readDirectory(input.contentDir, { recursive: true }))
    .filter(
      (entry) =>
        ...
        entry.endsWith("styal-resource-monitor"),
    )
  ...
  if (identity === "-") {
    yield* Effect.log("[cli-archive] Signed ad hoc (no STYAL_CLI_MAC_SIGN_IDENTITY).");
    return;
  }
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-styal.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized styal.");
});
...
/** Signs styal.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning styal.exe",
  );
  yield* Effect.log("[cli-archive] Signed styal.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "styal.exe" : "styal";
  ...
  if (cliArchivePlatformKey(input.platform, input.arch) !== `${hostPlatform}-${hostArch}`) {
    return yield* new CliArchiveInputMissingError({
      inputPath: `${input.platform}-${input.arch}`,
      hint: "Build and smoke-test each archive on its native platform and architecture.",
    });
  }
  ...
  yield* requireInput(builtExecutable, `Run \`node apps/server/scripts/cli.ts build-exe \` first.`);
  yield* requireInput(
    path.join(webClient, "index.html"),
    "Run `vp run --filter ./apps/server build` first.",
  );
  yield* requireInput(
    resourceMonitorDir,
  ...
  for (const icon of resolveWebIconOverrides(
    resolveWebAssetBrandForPackageVersion(input.version),
    "client",
  )) {
    yield* fs.copyFile(
      path.join(repoRoot, icon.sourceRelativePath),
      path.join(contentDir, icon.targetRelativePath),
    );
  }
