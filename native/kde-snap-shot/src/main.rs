...
#[zbus::interface(name = "build.styal.KdeCapture")]
impl ScriptReply {
    fn reply(
        &self,
        value: &str,
...
fn script(connection: &Connection, directory: &Path, body: &str) -> Result<String> {
    ...
    let source = format!(
        "function reply(value) {{ callDBus({destination}, '/build/styal/KdeCapture', 'build.styal.KdeCapture', 'Reply', JSON.stringify(value)); }}\ntry {{ {body} }} catch (error) {{ reply({{error: String(error)}}); }}"
    );
    ...
    connection
        .object_server()
        .remove::<ScriptReply, _>("/build/styal/KdeCapture")?;
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("activate") if args.len() == 5 => {
            ...
            if serde_json::from_str::<serde_json::Value>(&value)?["activated"] != true {
                return Err("KDE could not identify the styal window to activate".into());
            }
