...
describe("DesktopAppIdentity", () => {
  it.effect("resolves the user-data path from the app data directory", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        // styal owns a fresh directory; carrying state over from a previous
        // install is an explicit import rather than an implicit fallback.
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/styal");
      }),
    ),
  );
