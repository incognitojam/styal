...
export const make = Effect.gen(function* () {
  ...
  const resolveBranchHeadContext = Effect.fn("resolveBranchHeadContext")(function* (
    ...
    const remoteNameResult =
      details.remoteName !== undefined
        ? Result.succeed(details.remoteName)
        : yield* Effect.result(gitCore.readConfigValue(cwd, `branch.${details.branch}.remote`));
  ...
  const resolveLookupHeadContext = Effect.fn("resolveLookupHeadContext")(function* (
    ...
    // Name the own-name remote ref as the upstream so the configured merge ref,
    // which still points at the default branch, does not replace the head.
    const ownNameContext = yield* resolveBranchHeadContext(cwd, {
      branch: details.branch,
      upstreamRef: `${remoteName}/${details.branch}`,
      remoteName,
    });
