...
export const HYPRLAND_CAPTURE_EXECUTABLE = "styal-hyprland-snap-shot";
...
export function hyprlandCaptureExecutable(paths: HyprlandCapturePaths) {
  return NodePath.join(paths.dataHome, "styal", "hyprland-capture", HYPRLAND_CAPTURE_EXECUTABLE);
}
...
export class HyprlandCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        throw new Error(
          "The Hyprland capture helper is missing from this build. Update or reinstall styal.",
        );
