...
- Label a PR `preview:web` for a hosted web preview or `preview:mac` for an unsigned Apple Silicon DMG, rebuilt on every push. See [PR previews](docs/operations/development.md#pr-previews). When handing over a `preview:mac` build, link the PR's preview comment rather than the DMG itself: the comment carries the current download and the quarantine command the unsigned DMG needs.
