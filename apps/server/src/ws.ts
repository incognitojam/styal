...
import { resolveAddProjectDefaultDirectory } from "./project/AddProjectDefaultDirectory.ts";
      ...
      const loadServerConfig = (options: { readonly usageLimitsCommand: boolean }) =>
        Effect.gen(function* () {
          ...
          return {
            ...
            settings,
            addProjectDefaultDirectory: yield* resolveAddProjectDefaultDirectory(),
      ...
      return WsRpcGroup.of({
          ...
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              const loadThreadSnapshotItem = Effect.fn("ws.loadThreadSnapshotItem")(function* () {
                const snapshotItem = yield* loadThreadSnapshot();
                if (Option.isNone(snapshotItem)) {
                  return yield* threadNotFoundError();
                }
                return snapshotItem.value;
              });
              ...
              const projectThreadLiveItems = Effect.fn("ws.projectThreadLiveItems")(function* (
              ...
              const bufferedLiveStream = liveBuffer.stream.pipe(
                Stream.mapArrayEffect(projectThreadLiveItems),
              );
                ...
                if (
                  ...
                  const catchUpStream = orchestrationEngine
                    ...
                    .pipe(
                      ...
                      Effect.flatMap((events) => projectThreadEventBatch(events)),
              ...
              const snapshotItem = snapshot.value;
        ...
        [WS_METHODS.sourceControlListIssues]: (input) =>
          observeRpcEffect(
            ...
            sourceControlRepositories.listIssues(input),
            {
              "rpc.aggregate": "source-control",
            },
          ),
        [WS_METHODS.sourceControlGetIssue]: (input) =>
          observeRpcEffect(
            WS_METHODS.sourceControlGetIssue,
            sourceControlRepositories.getIssue(input),
            {
              "rpc.aggregate": "source-control",
            },
          ),
