# Drive styal from an agent

`styal drive` gives scripts and agents JSON access to running environments and repeatable synthetic conversation states. Use `styal drive --help` for commands and each subcommand's `--help` for flags.

## Control a running environment

Start an environment normally, then use the same styal home directory:

```sh
styal drive snapshot --home-dir /tmp/styal-demo
styal drive snapshot --home-dir /tmp/styal-demo --thread THREAD_ID
styal drive send --home-dir /tmp/styal-demo THREAD_ID "Verify the changes and report the results."
```
...
```sh
styal drive schema > command.schema.json
styal drive dispatch command.json --home-dir /tmp/styal-demo
```
...
To connect to a remote environment, replace `--home-dir` with `--url https://your-environment.example` and supply an existing environment bearer session in `T3_DRIVE_TOKEN`. Create one on the server's machine with `styal auth session issue`. Read operations require `orchestration:read`; mutations also require `orchestration:operate`. Keep tokens out of command files and source control. Local commands create and revoke their own temporary session.
...
```sh
styal drive example --workspace /path/to/project > scenario.json
styal drive schema --scenario > scenario.schema.json
styal drive serve scenario.json --home-dir /tmp/styal-drive-example
```
...
To generate a home without starting a server, use `styal drive scenario scenario.json --home-dir /tmp/styal-drive-offline`. Starting that home later with `styal serve --base-dir /tmp/styal-drive-offline` enables normal recovery, which changes orphaned running sessions into errors. Use `drive serve` for stable synthetic states.
