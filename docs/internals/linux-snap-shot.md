...
`<app-id>.SnapShot` on the session bus and exports `build.styal.SnapShot.Capture`; the config
...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@styal.build`. GNOME only discovers a newly
...
The extension trusts callers that own `build.styal.Styal.SnapShot` (or the `.Development`
