...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.styal-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        STYAL_HOME: "/home/user/.styal-test",
  ...
  it("keeps pull request preview state and identity apart from stable", () => {
    const options = resolveEarlyLinuxElectronOptions({
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.styal-test/preview/desktop-settings.json");
        return JSON.stringify({ linuxPasswordStore: "kwallet6" });
      },
    });
  ...
  it("keeps implicit development state under ~/.styal/dev when STYAL_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.styal/dev/desktop-settings.json");
  ...
  it("treats whitespace-only STYAL_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        STYAL_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
