  ...
  for (let page = 1; page <= RELEASE_INDEX_MAX_PAGES; page += 1) {
    const body = yield* httpClient
      ...
      .pipe(
        ...
        Effect.mapError(() => new CliUpdateError({ reason: "Could not list styal releases." })),
        Effect.timeoutOrElse({
          duration: RELEASE_INDEX_TIMEOUT,
          orElse: () =>
            Effect.fail(new CliUpdateError({ reason: "Timed out listing styal releases." })),
        }),
      );
    const releases = yield* decodeReleaseIndex(body).pipe(
      Effect.mapError(
        () => new CliUpdateError({ reason: "The styal release index had an unexpected shape." }),
      ),
    );
...
/** Whether a launcher target lives inside `<baseDir>/runtime/styal-executable/versions`. */
...
/**
 * The launcher the install scripts leave behind: a symlink at `<bin>/styal` on
 * POSIX, a `styal.cmd` shim on Windows. `styal update` repoints it so the next `styal`
 * invocation is the new version. Only a launcher that already points into
 * this home's `runtime/styal-executable/versions` tree is touched; a plain copy of the
...
export const repointLauncher = Effect.fn("cli.update.repoint_launcher")(function* (input: {
  ...
  /** `<baseDir>/runtime/styal-executable/versions` of the home being updated. */
  ...
  if (platform === "win32") {
    ...
    // itself as argv0; the shim is the `styal.cmd` next to it only when launched
    // from an install script's bin directory. Find it by searching the
    // directories that would resolve `styal` on this shell's PATH.
    ...
    yield* fs
      ...
      .pipe(
        Effect.mapError(
          () =>
            new CliUpdateError({ reason: `Could not rewrite the styal launcher at ${shimPath}.` }),
        ),
      );
  ...
  yield* fs.symlink(input.targetEntryPath, tempLink).pipe(
    ...
    Effect.mapError(
      () =>
        new CliUpdateError({
          reason: `Could not repoint the styal launcher at ${input.launchedAs}.`,
        }),
    ),
  );
 ...
 * spelling in argv0: a launcher symlink or `./styal` resolves against the
 * working directory, while a bare `styal` was found on PATH and has to be
 ...
 * only ever sees its own path. Walk PATH for a `styal.cmd` whose target is the
...
export const findWindowsShim = Effect.fn("cli.update.find_windows_shim")(function* (
  ...
  const environment = yield* HostProcessEnvironment;
  const candidates = [
    ...(environment["STYAL_INSTALL_BIN_DIR"] ? [environment["STYAL_INSTALL_BIN_DIR"]] : []),
    ...(environment["PATH"] ?? environment["Path"] ?? "").split(";"),
  ].filter((entry) => entry.trim().length > 0);
  for (const directory of candidates) {
    const shimPath = path.join(directory, "styal.cmd");
...
const updateFlags = {
  ...
  channel: Flag.choice("channel", CLI_RELEASE_CHANNELS).pipe(
    Flag.withDescription(
      "Release channel to follow. Defaults to the channel this styal was published on.",
    ),
    Flag.optional,
  ),
...
export const updateCommand = Command.make("update", {
  ...
  Command.withDescription(
    "Download a newer styal and switch this machine to it, including the background service when one is installed.",
  ),
...
/**
 * A `styal serve` or `styal` someone started by hand, as opposed to the one the
...
const belongsToBootService = Effect.fn("cli.update.belongs_to_boot_service")(function* (
  ...
  if (platform === "linux") {
    ...
    return Option.isSome(cgroup) && cgroup.value.includes("/styal.service");
  }
  if (platform === "darwin") {
...
const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  if (input.requestedVersion !== undefined && !isExactServiceVersion(input.requestedVersion)) {
    return yield* new CliUpdateError({
      reason: `'${input.requestedVersion}' is not an exact styal version.`,
    });
  }
  ...
  if (targetChannel === "preview" && currentChannel !== "preview") {
    yield* Console.log(
      [
        `styal@${targetVersion} is a preview build.`,
        ...
        `  will have to switch back to ${currentChannel} yourself with \`styal update --channel ${currentChannel} --allow-downgrade\`.`,
      ].join("\n"),
    );
  ...
  if (executableCurrent && serviceCurrent) {
    yield* Console.log(
      serviceVersion !== undefined
        ? `styal and its background service are already on ${targetVersion} (${targetChannel}).`
        : `styal is already on ${targetVersion} (${targetChannel}).`,
    );
    return;
  }
  if (!input.allowDowngrade && compareExactServiceVersions(targetVersion, newestInstalled) < 0) {
    return yield* new CliUpdateError({
      reason: `styal@${targetVersion} is older than the installed ${newestInstalled}. Pass --allow-downgrade to install it anyway.`,
    });
  }
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    if (input.assumeYes) {
      ...
      yield* Console.log(
        "  Not a terminal, so the service keeps running its current version. Rerun with --yes to restart it now, or run `styal service restart` later.",
      );
    }
  }
  ...
  const runtime = yield* ensurePinnedRuntimeInstalled({
    ...
    validate: (paths) =>
      runner
        ...
        .pipe(
          Effect.mapError(
            (cause) =>
              new PinnedRuntimeInstallError({ step: "verifying the downloaded styal", cause }),
          ),
            ...
            result.code === 0 && /\bv(\S+)\s*$/.exec(result.stdout)?.[1] === targetVersion
              ...
              : Effect.fail(
                  new PinnedRuntimeInstallError({
                    step: "verifying the downloaded styal",
    ...
    Effect.catchIf(
      (error): error is PinnedRuntimeInstallError =>
        ...
        error.step.startsWith("downloading the styal release checksums") &&
        String(error.cause).includes("404"),
      () =>
        Effect.fail(
          new CliUpdateError({
            reason: `No release archive was published for styal@${targetVersion}.`,
          }),
        ),
    ),
  );
  ...
  // is rewritten either way so a later `styal service restart` lands on the new
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    yield* BootService.BootService.pipe(
      ...
      Effect.catchTag("BootServiceMigrationRestartRequiredError", () =>
        Effect.sync(() => {
      ...
      Effect.mapError(
        (error) =>
          new CliUpdateError({
            reason: `styal@${targetVersion} is installed but the background service could not be ${restartService ? "updated" : "pointed at it"}: ${error.message}`,
          }),
      ),
    );
  ...
  progress.success(`Installed styal ${targetVersion}`);
  if (Option.isSome(repointed)) {
    yield* Console.log("  Run styal to get started.\n");
  } else {
  ...
  } else if (migrationDeferred) {
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`npx @styal/cli@${targetVersion} service update\` to migrate it when you are ready to restart.`,
    );
  } else if (serviceInstalled) {
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`styal service restart\` when you are ready to switch it to ${targetVersion}.`,
    );
