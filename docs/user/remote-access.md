...
Connect a phone, browser, or another desktop app to styal running on a different
machine. That machine must stay running and reachable while you work.

## styal Link

styal Link makes an environment available to your other devices without setting
up router forwarding. In the desktop app on the host, open **Settings →
Connections**, sign in, and enable **styal Link** for that environment.

For a command-line host, run:

```bash
npx @styal/cli@nightly link
```

Follow the sign-in instructions. Setup offers a
[background service](./background-service.md); if you decline it, start the
server with `npx @styal/cli serve`. Saving your sign-in alone does not make the
machine reachable.

On your other device, sign in to the same styal Link account and choose the
...
```bash
npx @styal/cli serve --host <private-ip>
```
...
[app.styal.build](https://app.styal.build) needs an HTTPS endpoint. It connects
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. styal starts
...
The remote host must be a macOS or Linux
[supported platform](./install.md#requirements) with `curl` or `wget`, plus
[provider setup](./install.md#providers). styal downloads the release matching
your desktop app to the host and verifies its checksum, so the host does not need
Node.js.

If SSH reconnecting fails after an app update, retry the launch once. Removing
the connection stops a server that styal launched; a server that was already
...
On a command-line host, `styal link unlink` disables exposure while retaining
your login; `styal link logout` also clears that login. Background-service
...
Run `styal link status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `styal service status` and read the displayed log. If it disappears
when SSH closes, see [background-service troubleshooting](./background-service.md#troubleshooting).

| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart styal on the host.                                                                           |
| `auth_invalid` or `invalid_bearer`                        | Run `styal link login`. If credentials were revoked, run `styal link logout`, then `styal link` again. Restart the server after signing in. |
| Expired or invalid link proof                             | Check the host's date and time, update styal, then restart it.                                                                              |
...
`systemctl --user restart styal.service` for the background service. For a
foreground server, stop it and run `styal serve` again with your usual options.
...
devices. For server version warnings, follow [Updating styal](./updating.md).
