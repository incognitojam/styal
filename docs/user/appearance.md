...
main local environment. app.styal.build and additional connections do not use them.
...
choosing another theme afterward sticks until the next `styal theme set`. Run the command again to
reapply it, even if the name is unchanged.

`styal theme clear` removes the default without changing anyone's current theme.
`styal theme show` lists the default and published themes.

### Publish a theme

Save a theme exported from T3 Code into `~/.styal/userdata/themes/` on the server, or the `themes`
directory under your custom state directory. The filename supplies the theme ID: `nightfall.json`
can be selected with `styal theme set nightfall`. Keep the filename stable when updating its colors.
