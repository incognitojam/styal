...
GitHub matches push path filters against only the first 300 changed files. A promotion that changes more files than that, as reference syncs under `.repos/` usually do, can skip **Deploy styal web** and **Mobile EAS Production** even when the batch changes the paths they watch. Compare `git diff --name-only <previous-main-SHA> <new-main-SHA> | wc -l` against 300, check which of those workflows ran for the new `main` SHA, and dispatch the missing ones from `main` when the maintainer asks. Production mobile builds use a fingerprint runtime version, so an OTA reaches installed builds only when the Expo fingerprint is unchanged. Dependency changes can move it without any native code change; compare `npx fingerprint fingerprint:generate --platform <ios|android>` in `apps/mobile` before and after the batch. If it moved, the JavaScript changes need new production builds (`mode=build`) rather than an OTA.
...
```bash
node apps/server/scripts/generate-t3-import-fixture.ts --server <upstream-checkout> --out apps/server/src/dataImport/testFixtures/t3-code
```
