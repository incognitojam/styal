...
styal reuses the active terminal when it is idle. If that terminal has a running process, it opens
...
Commands run in a styal terminal can open their browser target in the desktop Preview. This
...
styal opens the requested HTTP or HTTPS URL in the Preview panel for that terminal's thread. For
...
If no compatible desktop Preview is connected, styal delegates the request to the server
...
styal does not add `BROWSER` to your global shell configuration. It injects browser-opening
support into each terminal it creates.

In a newly created styal terminal, inspect the browser-opening environment with:
...
a styal helper directory. The exact paths vary by installation.
...
2. Confirm that the T3 server which owns the environment is up to date. Updating only the desktop
   or web client does not update a remote server, and running newer styal source inside a terminal
...
If `BROWSER` shows a value you configured, styal deliberately preserves it and does not install
