...
/** Checks the complete message before uploads or transferring draft ownership. */
export function attachmentBatchLimitError(
  attachments: ReadonlyArray<{ readonly sizeBytes: number }>,
): string | null {
  if (attachments.length > PROVIDER_SEND_TURN_MAX_ATTACHMENTS) {
    return `Messages can contain at most ${PROVIDER_SEND_TURN_MAX_ATTACHMENTS} attachments.`;
  }
  if (
    attachments.reduce((total, attachment) => total + attachment.sizeBytes, 0) >
    PROVIDER_SEND_TURN_MAX_TOTAL_ATTACHMENT_BYTES
  ) {
    return "Attachments can total at most 50 MB per message.";
  }
  return null;
}
