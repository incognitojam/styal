...
/** Transfer a completed upload to durable stash ownership without deleting its server bytes. */
export function forgetCompletedAttachmentUpload(imageId: string): void {
  if (!jobsByImageId.has(imageId) && readAttachmentUpload(imageId)?.status === "ready") {
    clearUploadState(imageId);
  }
}
