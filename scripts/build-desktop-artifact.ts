...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const binaryPath = path.join(
    ...
    rustTarget!,
    `release/styal-${input.backend}-snap-shot`,
  );
  if (!reuseHelpers) {
  ...
  const executable = path.join(destination, `styal-${input.backend}-snap-shot`);
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "styal captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "styal",
