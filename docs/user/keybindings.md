...
server, if you prefer editing it directly. styal writes the built-in defaults into that file on
...
inherits the project you were in — or, when no thread is open, the project the sidebar is filtered
to — along with model and mode selections. Branch, worktree, and
...
the app supplies today are `terminalFocus`, `terminalOpen`, `rightPanelFocus`, `previewFocus`,
`previewOpen`, and `modelPickerOpen`. The set is open and grows over time, so treat that as the
