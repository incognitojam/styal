...
import { CLI_COMMAND_NAME, CLI_PACKAGE_NAME } from "@t3tools/shared/cliPackage";
 ...
 * `@styal/cli@nightly`) is resolved away before our process starts, so re-derive it
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `${CLI_COMMAND_NAME} ${input.subcommand}`;
  }
