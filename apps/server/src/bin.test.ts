...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("exposes service lifecycle commands without styal Link configuration", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Manage the styal background service.");
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["styal", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["styal", "project", "add"]);
