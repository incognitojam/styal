...
/**
 * `styal theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `styal theme set`.
 *
...
export class ThemeFileInvalidError extends Schema.TaggedErrorClass<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from styal, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedErrorClass<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `styal theme clear` to remove the theme.";
  }
}

const envT3Home = Config.string("STYAL_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then STYAL_HOME,
  // then the default home. A provisioning script exporting STYAL_HOME must
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `styal theme set ocean` publish ./ocean whenever the cwd happens to
