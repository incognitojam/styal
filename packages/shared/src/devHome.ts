 ...
 * `~/.styal` that a user's installed styal runs against.
 *
 * A linked git worktree gets its own (gitignored) `.styal`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `STYAL_HOME` counts as an explicit base dir — flipping the state directory
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".styal");
  });
