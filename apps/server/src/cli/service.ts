...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      "styal service",
      `  Status: installed · styal@${installedVersion} (newer than this styal@${cliVersion} CLI)`,
      `  Unit: ${status.unitPath}`,
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`styal update ${installedVersion}\` to match it, or pass \`--allow-downgrade\` to \`styal service install\` explicitly.`,
    ].join("\n");
  }
  return [
    "styal service",
    `  Status: ${status.current ? `installed · styal@${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : ["  Next: Run `styal service install` to repair it."]),
  ].join("\n");
}
...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install styal as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `styal service is already installed with styal@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} styal service with styal@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
// Kept one release for muscle memory and old docs. It did what `styal service
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        yield* Console.log(
          "`styal service update` is deprecated: run `styal update` to move to a newer release, or `styal service install` to repair the service. Repairing now.",
        );
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`styal service is already using styal@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} styal service with styal@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceRestartCommand = Command.make("restart", projectLocationFlags).pipe(
  Command.withDescription(
    "Restart the background service. Picks up a version installed by `styal update` that was not restarted at the time.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the styal service${status.installedVersion === undefined ? "" : ` on styal@${status.installedVersion}`}.`
            : "styal service is not installed.",
        );
      }),
    ),
  ),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the styal background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the styal service." : "styal service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the styal background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("styal is already set up to run in the background on this machine.");
    return true;
  }
  ...
  if (
    ...
    yield* Console.log(
      `A newer styal@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  offer.pipe(
    Effect.catchTags({
      ...
      BootServiceUpdatePendingError: (error) =>
        Console.warn(`Background setup did not finish: ${error.message}`).pipe(Effect.as(false)),
      BootServiceMigrationRestartRequiredError: (error) =>
        Console.log(error.message).pipe(Effect.as(false)),
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the styal background service."),
