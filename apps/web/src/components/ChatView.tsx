...
export default function ChatView(props: ChatViewProps) {
  ...
  const displayServerMessages = useMemo<ReadonlyArray<ChatMessage>>(() => {
    ...
    return serverMessages.map((message) =>
      projectServerMessagePreviews(message, (attachment) =>
        attachment.type === "image" ? serverAttachmentUrlById.get(attachment.id) : undefined,
      ),
    );
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `styal:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's styal server to browse pull requests."
      />
